...
def calc_stats(config, item):
    """Calculate Progress for the given item."""
...
def calc_all_stats(config, items):
    """Parse all items to calculate their progress."""
    all_stats = []
    for item in items:
...
def resolve_material(material):
    """Mapping given material with GW2 One."""
...
def resolve_currency(currency):
    """Mapping given Currency with GW2 One."""
...
def get_all_characters_inventory():
    """Gather all characters inventory  and return all the items."""
    all_items = {}
    characters = client.characters.get()
    for character in characters:
        log(f"[4/8]: Retrieving character inventory of {character}...")
        url = (
            f"{client.BASE_URL}/v2/characters/{urllib.parse.quote(character)}/inventory"
        )
        inventory = client.characters.get(url=url)
        for bag in inventory["bags"]:
            for item in bag["inventory"]:
                if item:
                    if retrived_item := all_items.get(item["id"]):
                        retrived_item["count"] += item["count"]
                    else:
...
def verify_token():
    """Varify Token viability and permissions."""
    ...
    if not set(permissions_needed).issubset(permissions_given):
        raise ValueError(
            f"Insufficient permissions: Set your API key permissions "
            f"to at least: {str(permissions_needed)}"
        )


def load_config(configurations):
    """Load Asked configuration."""
    try:
        for conf_id, conf_name in enumerate(configurations):
            print(f"Choose ID: {conf_id} for {conf_name}.")
        chosen_configuration = int(
            input("Enter the ID corresponding the the configuration to load: ")
        )
        with open(f"./configurations/{configurations[chosen_configuration]}") as file:
            return json.load(file)
    except Exception:
        raise ValueError(f"Invalid config: {configurations[chosen_configuration]}")
...
def pretty_print_stats(all_stats, title="Name"):
    ...
    for stats in all_stats:
        table.add_row(
            [stats["name"], stats["value"], colored(stats["progress"], stats["color"])]
        )
    print(table)


def pause(pause_time):
    """Pause the process to match GW2 API refresh time."""
    while pause_time:
        mins, secs = divmod(pause_time, 60)
        log(f"(Refreshing in {mins:02d}:{secs:02d})")
        time.sleep(1)
        pause_time -= 1
    log("Refreshing...")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(
            colored("\rActive config: ", attrs=["bold"])
            + config["description"]
            + " by "
            + config["author"]
        )
